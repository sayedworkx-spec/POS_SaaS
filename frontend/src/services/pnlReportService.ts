...
const PNL_REPORT_CACHE_KEY = "pnl_report_snapshot";

function normalizeSnapshot(raw: any): PnLReportSnapshot {
  return {
    range: {
    ...
    inventoryBridge: {
      endingQty: Number(raw?.inventoryBridge?.endingQty ?? 0),
      endingInventoryValue: Number(raw?.inventoryBridge?.endingInventoryValue ?? 0),
      endingInventorySellValue: Number(raw?.inventoryBridge?.endingInventorySellValue ?? 0),
      inventoryPotentialMargin: Number(raw?.inventoryBridge?.inventoryPotentialMargin ?? 0),
      lowStockCount: Number(raw?.inventoryBridge?.lowStockCount ?? 0),
      zeroStockCount: Number(raw?.inventoryBridge?.zeroStockCount ?? 0),
      activeProducts: Number(raw?.inventoryBridge?.activeProducts ?? 0),
      inactiveProducts: Number(raw?.inventoryBridge?.inactiveProducts ?? 0),
    },
    statementLines: Array.isArray(raw?.statementLines)
      ? raw.statementLines.map((item: any) => ({
...
function readCache(): PnLReportSnapshot | null {
  const raw = localStorage.getItem(PNL_REPORT_CACHE_KEY);
  if (!raw) return null;
...
function writeCache(snapshot: PnLReportSnapshot) {
  localStorage.setItem(PNL_REPORT_CACHE_KEY, JSON.stringify(snapshot));
}

export function getPnLReportSnapshot(): PnLReportSnapshot | null {
  return readCache();
}

export async function syncPnLReportCache(startDate?: string, endDate?: string) {
  ...
  const response = await apiFetch(
    `/pnl-report/overview${query ? `?${query}` : ""}`
  );
